...
export default function MathLearningPlatform() {
  ...
  const tests = [
    {
      ...
      options: ['x = 2 и x = 3', 'x = -2 и x = -3', 'x = 1 и x = 6'],
      correct: 0
    },
    {
  ...
  const score = useMemo(() => {
    return tests.reduce((acc, test, idx) => {
      return acc + (answers[idx] === test.correct ? 1 : 0);
    }, 0);
  }, [answers, tests]);
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-8">
          ...
          <div className="space-y-4">
            {['Алгебра', 'Геометрия', 'Тригонометрия'].map((item, index) => (
              <div key={item}>
      ...
      <section id="practice" className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-5">
          <button
            ...
            className="px-6 py-3 rounded-2xl bg-blue-600 text-white font-semibold shadow hover:scale-105 transition"
          >
            Проверить ответы
          </button>
          ...
          <button
            type="button"
            onClick={resetTest}
            className="px-6 py-3 rounded-2xl bg-white border border-slate-300 font-semibold hover:bg-slate-100 transition"
          >
            Сбросить тест
          </button>
          ...
          {submitted && (
            <div className="text-lg font-semibold text-slate-700">
              Результат: {score} из {tests.length}
            </div>
          )}
        </div>
      </section>
